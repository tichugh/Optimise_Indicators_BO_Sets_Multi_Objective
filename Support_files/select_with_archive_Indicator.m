function Set_data = select_with_archive_Indicator(Data_non,Data_archive,no_sol,no_var,ref_point,Sets,B)


Data_non = find_nondominated(Data_non,Data_archive,no_var);
if size(Data_non,1)>no_sol
    %% sort with HV
    Set_data = sort_with_HV_Indicator(Data_non,no_sol,no_var,ref_point,B);
elseif size(Data_non,1)<no_sol
    %% select from the previous set
    Set_data = select_with_previous_set(Sets,Data_non,no_sol,ref_point,no_var,B);
else 
    data_temp = Data_non;
...
function Set_data = select_with_previous_set(Sets,Data_non,no_sol,ref_point,no_var,B)
...
non = P_sort(Set_data.F,'first')==1;
PF = Set_data.F(non,:);
HV_set = P_evaluate_indicator(PF,B);
Set_data.HV = HV_set;
end