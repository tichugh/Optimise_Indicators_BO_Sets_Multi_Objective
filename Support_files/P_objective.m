function [Output,Boundary,Coding] = P_objective(Operation,Problem,M,Input,K_problem)


    k = find(~isstrprop(Problem,'digit'),1,'last');
    switch Problem(1:k)
        case 'DTLZ'
            [Output,Boundary,Coding] = P_DTLZ(Operation,Problem,M,Input,K_problem);
        case 'SDTLZ'
        ...
        case 'ZDT'
            [Output,Boundary,Coding] = P_ZDT(Operation,Problem,Input,K_problem);
        case 'UF'
        ...
        case 'OneMaxContinous'
            corr = 0.5;
            [Output,Boundary,Coding] = P_OneMaxContinous(Operation,Problem,M,Input, corr);
        case 'Others'
...
function [Output,Boundary, Coding] = P_OneMaxContinous(Operation,Problem,M,Input,corr)
...
%             corr = -0.5;
            FunctionValue(:,1)  = sum(Population,2);
            for t = 1:size(Population,1)
                map = zeros(1,size(Population,2));
...
%                 y = mod(Population(t,:)+map,2);
                y = abs(map - Population(t,:));
%                 r = find(y==1);
%                 FunctionValue(t,2) = length(r);
            FunctionValue(t,2) = sum(y);
            end
...
%%
function [Output,Boundary,Coding] = P_DTLZ(Operation,Problem,M,Input,K_problem)
%     persistent K;
    Boundary = NaN; Coding = NaN;
    switch Operation
        %产生初始种群
...
function [Output,Boundary,Coding] = P_ZDT(Operation,Problem,Input,K_problem)
    persistent K;
    Boundary = NaN; Coding = NaN;
    switch Operation
        %产生初始种群
...
%              K = [8 8 8 8 8 8];
        ...
        case 'true'
            switch Problem
                ...
                case 'ZDT6'
%                     Population = zeros(Input,2);
                    Population(:,1) = linspace(0.2807753191,1,5000);
                    Population(:,2) = 1-Population(:,1).^2;
            end
            Output = Population;
        %计算收敛性
